...
describe('PrismaUserRepository', () => {
  ...
  beforeEach(async () => {
    ...
    const module: TestingModule = await Test.createTestingModule({
      providers: [PrismaUserRepository, { provide: PrismaService, useValue: mockPrismaService }],
    }).compile();
  ...
  describe('findByEmailVerificationToken', () => {
    ...
    it('should return null if token not found', async () => {
      ...
      const result = await repository.findByEmailVerificationToken('invalid-token');
  ...
  describe('findByPasswordResetToken', () => {
    it('should find user by password reset token', async () => {
      ...
      const result = await repository.findByPasswordResetToken('reset-token-123');
